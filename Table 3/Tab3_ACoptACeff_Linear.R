# Calculate AC-optimal design and AC-efficiency for the linear model using R package ICAOD
...
# Gradient vector of the model (linear in x)
fx <- function(x, param) {
  c(x)
}

# Information matrix for the linear model under SLSE
# param = c(theta, t1)
FIM_linear_SLSE <- function(x, w, param) {
  t1 <- param[2]
  g1 <- numeric(1)
  G2 <- matrix(0, nrow = 1, ncol = 1)
  for (i in 1:length(x)) {
    fi <- fx(x[i], param)
    g1 <- g1 + w[i] * fi
    G2 <- G2 + w[i] * (fi %*% t(fi))
...
# =========== Find locally tilde_c-optimal design for the linear model ============
res <- locally(
  fimfunc = FIM_linear_SLSE,
  lx = 0, ux = 150,
...
ACopt_weight <- function(x, w, param) {
  t1 <- param[2]
  fim <- FIM_linear_SLSE(x = x, w = w, param = param)$InfM
  M_inv <- solve(fim)
...
# For t = 0.9144
despoint_t0.9144 <- c(0, 150)
wstar_t0.9144 <- c(0.4531934, 0.5468066)
...
AC_eff <- function(x_true, w_true, param_true, x_mis, w_mis, param_mis) {
  ...
  M_inv_true <- solve(FIM_linear_SLSE(x = x_true, w = w_true, param = param_true)$InfM)
  M_inv_mis  <- solve(FIM_linear_SLSE(x = x_mis,  w = w_mis,  param = param_true)$InfM)
