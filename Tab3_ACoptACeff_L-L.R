# Calculate AC-optimal design and AC-efficiency for the log-linear model using R package ICAOD
...
t1_true <- 0.9144
themu <- c(0.07972, 1, 0.1)   # (theta1, theta2, mu)
E <- themu[3]
theta <- themu[1:2]

# Estimator for the defined dose level
delta_star <- exp(E / theta[1]) - theta[2]

# Determine tilde_c1 in equation (17) of Remark 2
c1 <- log(delta_star + theta[2])
c2 <- theta[1] / (delta_star + theta[2])
tilde_c1 <- c(c1, c2)

# Gradient vector of the model (log-linear)
fx <- function(x, param) {
  c(log(x + param[2]), param[1] / (x + param[2]))
}

# Information matrix for the log-linear model under SLSE
# param = c(theta1, theta2, t1)
FIM_loglinear_SLSE <- function(x, w, param) {
  t1 <- param[3]
  g1 <- numeric(2)
...
# =========== Find locally tilde_c-optimal design for the log-linear model ============
res <- locally(
  fimfunc = FIM_loglinear_SLSE,
  lx = 0, ux = 150,
...
ACopt_weight <- function(x, w, param) {
  t1 <- param[3]
  fim <- FIM_loglinear_SLSE(x = x, w = w, param = param)$InfM
  M_inv <- solve(fim)
...
# For t = 0
despoint_t0 <- c(0, 150)
wstar_t0 <- c(0.531465, 0.468535)
ACopt_weight(x = despoint_t0, w = wstar_t0, param = c(theta, 0))$AC_weight

# For t = 0.9144
despoint_t0.9144 <- c(0, 4.050728, 150)
wstar_t0.9144 <- c(0.3161034, 0.3523281, 0.3315684)
...
AC_eff <- function(x_true, w_true, param_true, x_mis, w_mis, param_mis) {
  ...
  M_inv_true <- solve(FIM_loglinear_SLSE(x = x_true, w = w_true, param = param_true)$InfM)
  M_inv_mis  <- solve(FIM_loglinear_SLSE(x = x_mis,  w = w_mis,  param = param_true)$InfM)
