...
d <- E * theta[2] / (theta[1] - E)

# Determine tilde_c1 in equation (17) of Remark 2
c1 <- d / (theta[2] + d)
c2 <- -theta[1] * d / (theta[2] + d)^2
tilde_c1 <- c(c1, c2)

# Gradient vector of the model
fx <- function(x, param) {
...
# True t = 0.2, misspecified as t = 0
AC_eff(x_true = despoint_t0.2, w_true = wstar_t0.2, param_true = c(theta, 0.2),
       x_mis = despoint_t0,     w_mis = wstar_t0,    param_mis = c(theta, 0))

# True t = 0.7, misspecified as t = 0
AC_eff(x_true = despoint_t0.7, w_true = wstar_t0.7, param_true = c(theta, 0.7),
       x_mis = despoint_t0,     w_mis = wstar_t0,    param_mis = c(theta, 0))

# True t = 0.9, misspecified as t = 0
AC_eff(x_true = despoint_t0.9, w_true = wstar_t0.9, param_true = c(theta, 0.9),
       x_mis = despoint_t0,     w_mis = wstar_t0,    param_mis = c(theta, 0))